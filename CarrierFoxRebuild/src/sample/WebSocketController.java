...
public class WebSocketController extends WebSocketClient {
    ...
    private final int WRAPPINGWIDTH = 300;
    ...
    private Parent normalWrapping(Message message) throws IOException
    {
        ...
        if (stringWidth > WRAPPINGWIDTH)
        {
            messageHBoxWrapper.setPrefWidth(WRAPPINGWIDTH);
            messageTemplate.setPrefWidth(WRAPPINGWIDTH);
            messageWrapper.setPrefWidth(WRAPPINGWIDTH);
            messageText.setWrappingWidth(WRAPPINGWIDTH);
        }
        else
        {
        ...
        int numberOfLines = (int) Math.round(stringWidth / WRAPPINGWIDTH) + 1;
